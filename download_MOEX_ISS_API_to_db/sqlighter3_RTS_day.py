"""
Создание БД с таблицей дневных котировок фьючерса RTS
...
def non_empty_table_futures(connection, cursor):
    """Проверяем, есть ли записи в таблице 'Day' в БД"""
    with connection:
        return cursor.execute("SELECT count(*) FROM (select 1 from Day limit 1)").fetchall()[0][0]


def tradedate_futures_exists(connection, cursor, tradedate):
    """Проверяем, есть ли указанная дата в таблице 'Day' в БД"""
    with connection:
        result = cursor.execute('SELECT * FROM `Day` WHERE `TRADEDATE` = ?', (tradedate,)).fetchall()
        return bool(len(result))


def add_tradedate_future(connection, cursor, tradedate, secid, open, low, high, close, volume, openposition, shortname,
                         lsttrade):
    """Добавляет строку в таблицу Day """
    with connection:
        return cursor.execute(
            "INSERT INTO `Day` (`TRADEDATE`, `SECID`, `OPEN`, `LOW`, `HIGH`, `CLOSE`, `VOLUME`, `OPENPOSITION`, "
...
def get_max_date_futures(connection, cursor):
    ...
    with connection:
        return cursor.execute('SELECT MAX (TRADEDATE) FROM Day').fetchall()[0][0]
...
if __name__ == '__main__':  # Создание БД, если её не существует
    ...
    file_bd: str = f'{tiker}_futures_day_pj1.db'
