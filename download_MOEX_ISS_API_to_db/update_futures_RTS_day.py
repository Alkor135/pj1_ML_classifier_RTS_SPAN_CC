        ...
        while tradedate != today_date:
            ...
            if not sqlighter3_RTS_day.tradedate_futures_exists(connection, cursor, tradedate):
                ...
                if len(df) != 0:  # Если полученный ответ не нулевой, чтобы исключить выходные дни
                    ...
                    # Убираем строки, где дата последних торгов больше даты экспирации или равна дате экспирации
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    tiker: str = 'RTS'
    path_db: Path = Path(fr'c:\Users\Alkor\gd\data_quote_db\{tiker}_futures_options_pj1.db')
